...
const OptionsAndCriteriaSlice = createSlice({
	...
	initialState,
	reducers: {
		setDecisionOptions(state: OptionsAndCriteriaState, action: PayloadAction<OptionAndCriteria[]>) {
		...
		updateDecisionOption(state, action: PayloadAction<OptionAndCriteria>) {
			state.decisionOptions = state.decisionOptions.map(item => (item.id === action.payload.id ? action.payload : item));
		},
		deleteDecisionOption(state, action: PayloadAction<number>) {
			state.decisionOptions = state.decisionOptions.filter(item => item.id !== action.payload);
		},
		...
		deleteSelectionCriteria(state, action: PayloadAction<number>) {
			state.selectionCriteria = state.selectionCriteria.filter(item => item.id !== action.payload);
		},
	},
});
